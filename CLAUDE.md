...
- Wire into `src/app/demo/page.tsx` via `sideMenuGroups` (nav entry) and the `sectionContent` map (`id: <NameDemo />`) — do not add new `{activeSection === "x" && ...}` conditional blocks; that pattern was retired. Each section renders inside a Tabs "Interface"/"Demo" split; use `interfaceOverrides` for sections with no single Props interface (see README).
...
- Theme colors: reuse existing `--color-*` tokens already defined in `src/styles/profiles/base/index.css` (shared derived tokens) and `src/styles/profiles/*/index.css` (per-profile base palette: background, accent, neutral, success, warning, error, info, text-primary, bg-elevated, white) before introducing a new CSS variable. Profiles: `dev`, `gravitova`, `calma`.
- Fonts: `--font-major` (UI text) and `--font-fira` (code) are defined once in `src/styles/fonts.css` (Google Fonts default). Never hardcode a `font-family` in component CSS — use these two variables. See README → "Updating fonts" before changing either.
- Decorative corner-bracket accents (`CornerBrackets` in `src/components/corner/`) are dev-profile-only — all their CSS lives in `src/styles/profiles/dev/corner.css`, scoped under `[data-profile="dev"]`. They render nothing in `gravitova`/`calma`. Reuse the existing `.corner`/`.corner--*` classes via `<CornerBrackets />` rather than hand-writing new corner markup.
