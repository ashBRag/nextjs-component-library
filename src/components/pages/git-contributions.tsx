...
export default function GitContributions() {
  ...
  const contributionWeeks =
    contributions.length > 0 ? getContributionMatrix() : [];
  ...
  function getMonthLabels() {
    ...
    if (contributionWeeks.length > 0) {
      ...
      flatMatrix.forEach((day) => {
        ...
        if (
          !months.find(
            (m) => m.name === monthName && m.year === date.getFullYear(),
          )
        ) {
          months.push({
            name: monthName,
  ...
  const getContributionStats = () => {
    ...
    const highestDay = contributions.reduce(
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-white p-4 rounded-lg shadow-md">
                ...
                {stats.longestStreakDates?.length > 0 && (
                  <p className="text-sm text-gray-500 mt-1">
                    {formatDate(stats.longestStreakDates[0])} -{" "}
                    {formatDate(
                      stats.longestStreakDates[
                        stats.longestStreakDates.length - 1
                      ],
                    )}
                  </p>
                )}
              ...
              <div className="bg-white p-4 rounded-lg shadow-md">
                ...
                <p className="text-3xl font-bold text-indigo-600">
                  {stats.highestDay?.count || 0} contributions
                </p>
