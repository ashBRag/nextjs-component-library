...
export default function VerticalTabs({
  ...
  return (
    <div className={`md:flex ${className}`}>
      {/* Tabs List */}
      <ul
        className={`flex-column space-y-4 text-sm font-medium text-gray-500 dark:text-gray-400 md:me-4 mb-4 md:mb-0 ${tabsClassName}`}
      >
        {tabs.map((tab) => (
      ...
      {/* Tab Content */}
      <div
        className={`p-6 bg-gray-50 text-medium text-gray-500 dark:text-gray-400 dark:bg-gray-800 rounded-lg w-full ${contentClassName}`}
      >
        {activeTabContent}
      </div>
    </div>
