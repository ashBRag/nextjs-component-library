...
interface UndertaleTab {
  ...
  soulType?: 'determination' | 'kindness' | 'justice' | 'bravery' | 'patience' | 'integrity' | 'perseverance';
...
export default function UndertaleTabs({ 
  ...
  const getCharacterTabStyles = (tab: UndertaleTab, isActive: boolean) => {
    ...
    const colorStyle = tab.character ? characterColors[tab.character] : defaultColor;
    const animationStyle = animated ? "transition-all duration-300 ease-out" : "";
  ...
  return (
    <div className={`backdrop-blur-sm border-2 rounded-lg ${getThemeStyles()} ${className}`}>
      ...
      <div className="mb-0 border-b-2 border-slate-700/50 relative">
        <ul className="flex flex-wrap text-sm font-medium text-center relative z-10" role="tablist">
          {tabs.map((tab) => {
              ...
              <li key={tab.id} className="relative" role="presentation">
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    {tab.icon && (
                      <span className={animated && isActive ? "animate-pulse" : ""}>
                        {tab.icon}
                      </span>
                    )}
                    <span className={getCharacterLabelStyles(tab.character)}>
                      {getCharacterPrefix(tab.character, isActive)}{tab.label}{getCharacterSuffix(tab.character, isActive)}
                    </span>
                  </div>
