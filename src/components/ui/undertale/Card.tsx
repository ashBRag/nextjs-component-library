...
interface UndertaleCardProps {
  ...
  variant?: 'papyrus' | 'sans' | 'flowey' | 'frisk' | 'toriel' | 'undyne' | 'alphys' | 'mettaton';
  size?: 'sm' | 'md' | 'lg';
  animated?: boolean;
  showBorder?: boolean;
}
...
export default function Card({ 
  ...
  const getVariantStyles = () => {
    const variants = {
      ...
      // Papyrus - orange/warm
      papyrus: 'bg-orange-900 border-orange-400 text-orange-100 shadow-orange-400/20',
      
      // Flowey - dark/sinister
      flowey: 'bg-gray-900 border-yellow-500 text-yellow-100 shadow-yellow-500/30',
      
      // Frisk - determination red
      frisk: 'bg-red-950 border-red-500 text-red-100 shadow-red-500/25',
      
      // Toriel - warm purple/motherly
      toriel: 'bg-purple-900 border-purple-400 text-purple-100 shadow-purple-400/20',
      
      // Undyne - green/heroic
      undyne: 'bg-green-900 border-green-400 text-green-100 shadow-green-400/20',
      
      // Alphys - yellow/scientific
      alphys: 'bg-yellow-900 border-yellow-400 text-yellow-100 shadow-yellow-400/20',
  ...
  const getAnimationStyles = () => {
    ...
    const animations = {
      ...
      papyrus: 'hover:scale-105 hover:shadow-orange-400/40 hover:-rotate-1 transition-all duration-300',
      flowey: 'hover:scale-110 hover:shadow-yellow-500/50 transition-all duration-200 hover:brightness-110',
      frisk: 'hover:scale-105 hover:shadow-red-500/40 transition-all duration-300',
      toriel: 'hover:scale-105 hover:shadow-purple-400/40 transition-all duration-300 ease-in-out',
      undyne: 'hover:scale-105 hover:shadow-green-400/40 hover:rotate-1 transition-all duration-200',
      alphys: 'hover:scale-105 hover:shadow-yellow-400/40 transition-all duration-300',
      mettaton: 'hover:scale-110 hover:shadow-pink-400/50 hover:rotate-2 transition-all duration-300 ease-out'
    };
    ...
    `}>
      ...
      <div className="relative z-10">
        <h5 className={`
          ...
          ${titleClassName}
        `}>
          {variant === 'papyrus' ? '* ' : ''}{title}{variant === 'sans' ? '...' : ''}
        </h5>
        
        {typeof description==='string' ? <p className={`
          font-normal leading-relaxed
          ${descriptionClassName}
        `}>
          {variant === 'flowey' ? '* ' : ''}{description}
        </p>: description}
