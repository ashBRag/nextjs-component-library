  ...
  return (
    <ol className={`relative border-s border-gray-200 dark:border-gray-700 ${className}`}>
      {items.map((item, index) => (
        <li key={item.id} className={`${index === items.length - 1 ? 'ms-6' : 'mb-10 ms-6'}`}>
          {/* Timeline Icon */}
          ...
          <div className="flex items-center mb-1 flex-wrap">
            ...
            {item.badge && (
              <span className={`text-sm font-medium me-2 px-2.5 py-0.5 rounded-sm ms-3 ${getBadgeStyles(item.badge.variant)}`}>
                {item.badge.text}
              </span>
            )}
          </div>
