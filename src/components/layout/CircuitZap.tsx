    ...
    const createZap = () => {
      ...
      const paths = [
        ...
        { segments: [
          ...
          { x: startX + gridSize * 4, y: startY, width: 2, height: gridSize * 3, dir: 'v' }
        ]},
        // Vertical then horizontal
        { segments: [
          { x: startX, y: startY, width: 2, height: gridSize * 3, dir: 'v' },
          { x: startX, y: startY + gridSize * 3, width: gridSize * 5, height: 2, dir: 'h' }
        ]},
        // Long horizontal
        { segments: [{ x: startX, y: startY, width: gridSize * 8, height: 2, dir: 'h' }] },
        // Long vertical
        { segments: [{ x: startX, y: startY, width: 2, height: gridSize * 6, dir: 'v' }] },
        // U-shape
        { segments: [
          { x: startX, y: startY, width: 2, height: gridSize * 4, dir: 'v' },
          { x: startX, y: startY + gridSize * 4, width: gridSize * 3, height: 2, dir: 'h' },
          { x: startX + gridSize * 3, y: startY + gridSize * 2, width: 2, height: gridSize * 2, dir: 'v' }
        ]}
      ];
      
      const selectedPath = paths[Math.floor(Math.random() * paths.length)];
      const color = ['#00d4ff', '#00ff88', '#a855f7', '#ff0088'][Math.floor(Math.random() * 4)];
